...
def drop_random(width, gridworld, num_colors, blocksize):
    ...
    if orientation == HORIZONTAL:
        possible_locs = [
            i
            for i in range(width - blocksize + 1)
            if np.sum(gridworld[0, i : i + blocksize]) == 0
        ]
        if possible_locs:
            ...
            possible_locs = [
                i for i in range(width) if np.sum(gridworld[:blocksize, i]) == 0
            ]
            if possible_locs:
                x = np.random.choice(possible_locs)
            else:
...
def gravity(gridworld, orientation, x, blocksize):
    ...
    if (
        orientation == HORIZONTAL
        and np.sum(gridworld[0, x : x + blocksize]) != 0
    ) or (orientation == VERTICAL and np.sum(gridworld[:blocksize, x]) != 0):
        return -1
