...
from blockworld_utils import HORIZONTAL, VERTICAL, generate_random_world, drop_random, place, gravity, matches
...
class BlockWorldSingleEnv(gym.Env):
    ...
    def __init__(self, gridlen, num_blocks, num_colors, blocksize):
        ...
        gridformat = [self.num_colors+1]*self.gridlen*self.gridlen
        
        self.observation_space = gym.spaces.Dict({
                                    'observation': gym.spaces.MultiDiscrete(gridformat),
                                    'desired_goal': gym.spaces.MultiDiscrete(gridformat)
                                })
        ## we don't need a no-op do we
        self.action_space = gym.spaces.Discrete(self.gridlen * self.num_orientations * self.num_colors)
        self.viewer = None
    ...
    def reset(self):
        while True:
            ...
            self.goal = generate_random_world(self.gridlen, self.num_blocks, self.num_colors, self.blocksize)
            if self.goal is not None:
                break
        self.grid = np.zeros((self.gridlen, self.gridlen))
        self.num_steps = 0
        self.viewer = None
        return {'observation': self.grid.copy().flatten(), 'desired_goal': self.goal.copy().flatten()}
    ...
    def step(self, action):
        ...
        return {'desired_goal': self.goal.copy().flatten(), 'observation': self.grid.copy().flatten()}, reward, done, {'is_success': is_success}
    ...
    def encode_action(self, x, orientation, color):
        assert x < self.gridlen and orientation < self.num_orientations and color in [BLUE, RED][:self.num_colors]
        color -= 1
        action = x
    ...
    def decode_action(self, action_int):
        ...
        color += 1
        assert color in [BLUE, RED][:self.num_colors]
...
if __name__ == '__main__':
    env = BlockWorldSingleEnv(gridlen=4, num_blocks=5, num_colors=2, blocksize=2)
    print(env.reset())